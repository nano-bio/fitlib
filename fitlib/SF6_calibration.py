...
from fitlib import loglib
import re
from scipy import *

from fitlib.fitlib import plot_fit, fit_function_to_data, guess_ES_peaks, gaussfunctions, plotES, fitES
...
def do_CCl4_calibration(filename, showplots=True, outputfile='CCl4_cal.pdf'):
    ...
    if badfile is not True:
        # guess peaks
        peaks = guess_ES_peaks(data, 1, offset=None, limit=1)
        ...
        # fit it
        peaksfound, p = fitES(data, peaks, linear_addition=False)
        log.write('Result of peak fitting: ' + str(peaksfound))

        # plot to get a nice file
        plotES(data, 'CCl4')
        plot_fit(data, gaussfunctions(2, False), p)
...
def do_SF6_calibration(filelist, showplots=True, quadratic=True, outputfile='SF6_cal.pdf'):
    ...
    frag_info = {'SF6': [[0.0], [0], [], None, [7.0]], 'SF5': [[0.1], [0], [], None, [7.0]],
                 'F': [[5.5, 9.0, 11.5], [0, 0, 0], [], [5.0], [16.0]],
                 'F2': [[4.625, 11.49], [0, 0], [], None, [15.0]]}
    ...
    for file in filelist:
        if fn_pattern.search(file):
            ...
            if badfile is False:
                ...
                # guess peaks
                peaks = guess_ES_peaks(data, len(frag_info[fragment][0]), offset=frag_info[fragment][3],
                                       limit=frag_info[fragment][4])
                log.write('Result of peak guessing: ' + str(peaks))
                # use guessed values to fit; return peak list to frag_info and add fit function parameters to frag_info
                if fragment is 'F':
                    frag_info[fragment][1], frag_info[fragment][2] = fitES(data, peaks, True)
                else:
                    frag_info[fragment][1], frag_info[fragment][2] = fitES(data, peaks)
                n = 0
                ...
                if type(frag_info[fragment][2]) is not list:
                    plt.subplot(3, 2, i)
                    plotES(data, fragment)
                    # the /3 stems from the fact that the function takes the number of peaks, not parameters
                    if fragment is 'F':
                        plot_fit(data, gaussfunctions(3, True), frag_info[fragment][2])
                    else:
                        plot_fit(data, gaussfunctions(len(frag_info[fragment][2]) / 3), frag_info[fragment][2])
                    i = i + 1
    ...
    print(fitdata)
    parameters = fit_function_to_data(fitdata, fitfunc, p)
    ...
    plot_fit(fitdata, fitfunc, parameters)
